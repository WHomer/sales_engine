...
describe do
  describe "GET #index, merchants revenue" do
    it "returns the total revenue for date x across all merchants" do
      ...
      get "/api/v1/merchants/revenue?date=2019-06-30"

      expect(response).to have_http_status(:success)

      results = JSON.parse(response.body)["data"]["attributes"]
      expect(results["total_revenue"]).to eq("5527.2")
    end
  end

  describe "GET #show, merchants revenue" do
    it "returns the revenue for a merchant" do
      ...
      results = JSON.parse(response.body)["data"]["attributes"]
      expect(results["revenue"]).to eq("3684.8")

      get "/api/v1/merchants/#{m1.id}/revenue?date=2019-06-30"
